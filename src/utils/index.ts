...
import { DOTS, GRID_GAP, GRID_MARGINE_LEFT_RIGHT, PAGINATION_HEIGHT, SEARCH_HEIGHT, THUMB_CARD_HEIGHT, THUMB_CARD_WEIGHT } from "../constants";
...
export const useImagesPerPage = () => {
    const calculateImagesPerPage = () => {
        const height = ((window.innerHeight - ((SEARCH_HEIGHT + PAGINATION_HEIGHT) / devicePixelRatio)) * devicePixelRatio) / ((THUMB_CARD_HEIGHT + GRID_GAP) * devicePixelRatio);
